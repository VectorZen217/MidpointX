...
export class GoogleCalendarConnector implements IConnector {
  ...
  getTools(): ConnectorTool[] {
    return [
      ...
      { name: "create_event", description: "Create a new calendar event", inputSchema: { type: "object", properties: { title: { type: "string" }, start: { type: "string" }, end: { type: "string" } }, required: ["title", "start", "end"] }, execute: async () => { throw new Error(OAUTH_NOT_READY); } },
